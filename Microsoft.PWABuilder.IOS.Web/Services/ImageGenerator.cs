...
namespace Microsoft.PWABuilder.IOS.Web.Services
{
    ...
    public class ImageGenerator
    {
        ...
        public async Task<ImageGeneratorResult> Generate(IOSAppPackageOptions.Validated options, WebAppManifestContext manifest, string outputDirectory)
        {
        ...
        private async Task<byte[]> GetBaseImage(IOSAppPackageOptions.Validated options, WebAppManifestContext webManifest)
        {
            ...
            var baseImageSources = new[]
            {
                ...
                (source: webManifest.GetIconSuitableForIOSApps(1024), description: "the largest square PNG icon 1024x1024 or larger from web manifest"),
                (source: webManifest.GetIconSuitableForIOSApps(512), description: "the largest square PNG icon 512x512 or larger from web manifest"),
                (source: webManifest.GetIconSuitableForIOSApps(256), description: "the largest square PNG icon 256x256 or larger from web manifest"),
                (source: webManifest.GetIconSuitableForIOSApps(128), description: "the largest square PNG icon 128x128 or larger from web manifest"),
                (source: webManifest.GetIconSuitableForIOSApps(0), description: "the largest square PNG icon from web manifest")
            };
            ...
            throw new InvalidOperationException($"Couldn't find a suitable base image from which to generate all Windows package images. Please ensure your web app manifest has a square PNG image 512x512 or larger. Base image sources: {string.Join(", ", baseImageSources)}");
        }
        ...
        private IEnumerable<ImageSource> GetImageSourcesForTargetSizes(WebAppManifestContext webManifest, ImageGeneratorServiceZipFile generatedImagesZip)
        {
            ...
            foreach (var source in sources)
            {
                ...
                if (filePath != null)
                {
                    ...
                    if (source.Size.IsLaunchIconSize())
                    {
                        ...
                        File.Copy(filePath, Path.Combine(outputDirectory, launchIconFileName), overwrite: true);
                    }
                }
            }
        ...
        private async Task<string?> TryWriteImageSourceToDirectory(ImageSource source, string outputDirectory)
        {
            ...
            foreach (var (action, description) in streamOpeners)
            {
                ...
                if (stream != null)
                {
                    var filePath = await TryWriteStreamToOutputDirectory(stream, source.TargetFileName, appIconDirectory, description ?? string.Empty);
                    if (filePath != null)
                    {
                        return filePath;
                    }
